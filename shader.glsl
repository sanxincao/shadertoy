...
vec3 getSky(vec3 ro, vec3 rd, vec3 lp){
    ...
    vec3 p = (ro + rd*FAR)/1. + vec3(0, 0, u_time);
...
void main(){
	...
	// Camera Setup.
	vec3 ro = vec3(0, 1.25, u_time*2.); // Camera position, doubling as the ray origin.
    ...
    */
    ...
    float overlay = 1. + .35*sin(u2.x*3.14159*u_resolution.y/1.5);
    overlay *= 1. + .35*sin(u2.y*3.14159*u_resolution.y/1.5); 
    ...
    */
	...
	gl_FragColor = vec4(sqrt(max(sceneColor, 0.)), 1);
}
